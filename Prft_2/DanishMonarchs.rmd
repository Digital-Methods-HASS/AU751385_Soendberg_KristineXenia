...
kings1 <- read.csv("data/Monarkiet.csv")

kings2 <- read_csv("data/Monarkiet.csv")

kings3 <- read.csv2("data/Monarkiet.csv")

kings4 <- read_csv2("data/Monarkiet.csv")
```
...
4. Show the dataset so that we can see how R interprets each column
  ...
  I have used the glimpse(kings) and the head(kings) function.

```{r eval = TRUE}
# COMPLETE THE BLANKS BELOW WITH YOUR CODE, then turn the 'eval' flag in this chunk to TRUE.

kings <- read_csv2("data/Monarkiet.csv", na = c("n/a"))
glimpse(kings)
